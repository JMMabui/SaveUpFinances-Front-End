...
import type { debtPaymentsRequest, debtPaymentsResponse } from "./Type/debts-payments.type";
...
export function useGetDebtPaymentsByDebt(debtId: string) {
  return useQuery({
    ...
    queryFn: async () => apiService.get<debtPaymentsResponse[]>(`${BASE}/debt/${debtId}`),
    enabled: !!debtId,
  });
}
...
export function useCreateDebtPayment() {
  ...
  return useMutation({
    ...
    mutationFn: async (data: debtPaymentsRequest) => apiService.post<debtPaymentsResponse>(BASE, data),
    onSuccess: () => {
...
export function useUpdateDebtPayment(id: string) {
  ...
  return useMutation({
    ...
    mutationFn: async (data: Partial<debtPaymentsRequest>) => apiService.put<debtPaymentsResponse>(`${BASE}/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["debts-payments"] });
      queryClient.invalidateQueries({ queryKey: ["debts-payments", "by-id", id] });
    },
  });
}
