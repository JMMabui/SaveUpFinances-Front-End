...
import type { accountBalanceRequest, accountBalanceResponse } from "./Type/account-balance.type";
...
export function useGetAccountBalanceByAccount(accountId: string) {
  return useQuery({
    ...
    queryFn: async () => apiService.get<accountBalanceResponse[]>(`${BASE}/account/${accountId}`),
    enabled: !!accountId,
  });
}
...
export function useGetAccountBalanceById(id: string) {
  return useQuery({
    ...
    queryFn: async () => apiService.get<accountBalanceResponse>(`${BASE}/${id}`),
    enabled: !!id,
  });
}
...
export function useCreateAccountBalance() {
  ...
  return useMutation({
    ...
    mutationFn: async (data: accountBalanceRequest) => apiService.post<accountBalanceResponse>(BASE, data),
    onSuccess: () => {
...
export function useUpdateAccountBalance(id: string) {
  ...
  return useMutation({
    ...
    mutationFn: async (data: Partial<accountBalanceRequest>) => apiService.put<accountBalanceResponse>(`${BASE}/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["account-balance"] });
      queryClient.invalidateQueries({ queryKey: ["account-balance", "by-id", id] });
    },
  });
}
