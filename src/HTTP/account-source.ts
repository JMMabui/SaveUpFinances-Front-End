...
import type { accountSourceRequest, accountSourceResponse } from "./Type/account-source.type";
...
export function useGetAccountSourcesByAccount(accountId: string) {
  return useQuery({
    ...
    queryFn: async () => apiService.get<accountSourceResponse[]>(`${BASE}/account/${accountId}`),
    enabled: !!accountId,
  });
}
...
export function useCreateAccountSource() {
  ...
  return useMutation({
    ...
    mutationFn: async (data: accountSourceRequest) => apiService.post<accountSourceResponse>(BASE, data),
    onSuccess: () => {
...
export function useUpdateAccountSource(id: string) {
  ...
  return useMutation({
    ...
    mutationFn: async (data: Partial<accountSourceRequest>) => apiService.put<accountSourceResponse>(`${BASE}/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["account-source"] });
      queryClient.invalidateQueries({ queryKey: ["account-source", "by-id", id] });
    },
  });
}
