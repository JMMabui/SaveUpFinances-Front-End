        ...
        <div className="space-y-4">
          {goals.map((goal: any) => {
            ...
            return (
              <div
                key={goal.id}
                className="space-y-3 p-3 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors"
              >
                <div className="flex items-center justify-between">
                  <h4 className="font-medium text-gray-900">{goal.name}</h4>
                  <div
                    className={`px-2 py-1 rounded-full text-xs font-medium ${progress >= 100 ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}
                  >
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">
                    {goal.currentAmount.toLocaleString()} Mt /{' '}
                    {goal.targetAmount.toLocaleString()} Mt
                  </span>
