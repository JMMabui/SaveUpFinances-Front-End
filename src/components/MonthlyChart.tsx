...
export const MonthlyChart = ({ transactions, filter, month }: MonthlyChartProps) => {
  if (month === -1) {
    // Mostrar todos os meses
    // Gerar arrays de totais por mês
    const incomeData = months.map((_, idx) =>
      transactions.filter(t => t.type === 'income' && t.month === idx).reduce((acc, t) => acc + t.amount, 0)
    );
    const expenseData = months.map((_, idx) =>
      transactions.filter(t => t.type === 'expense' && t.month === idx).reduce((acc, t) => acc + t.amount, 0)
    );
    const investmentData = months.map((_, idx) =>
      transactions.filter(t => t.type === 'investment' && t.month === idx).reduce((acc, t) => acc + t.amount, 0)
    );
  ...
  const investment = monthTransactions.filter(t => t.type === 'investment').reduce((acc, t) => acc + t.amount, 0);
