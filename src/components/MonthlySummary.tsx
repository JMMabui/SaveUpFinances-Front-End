...
export function MonthlySummary() {
  ...
  const incomes = (incomeData?.data || []).map((i: any) => ({
    ...i,
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <p className="text-sm text-gray-600">Comparação com o mês anterior</p>
      </CardHeader>
      <CardContent>
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <div
                className={`flex items-center justify-center gap-1 text-xs mt-1 ${getChangeColor(getChangePercentage(currentMonth.income, previousMonth.income))}`}
              >
                {getChangeIcon(
                  getChangePercentage(currentMonth.income, previousMonth.income)
                )}
                <span>
                  {getChangePercentage(
                    currentMonth.income,
            ...
            >
              ...
              <div
                className={`flex items-center justify-center gap-1 text-xs mt-1 ${getChangeColor(getChangePercentage(currentMonth.expenses, previousMonth.expenses))}`}
              >
                {getChangeIcon(
                  getChangePercentage(
          ...
          <div className="space-y-3">
            <div className="flex justify-between text-sm text-gray-600">
              ...
              <span className="font-medium">
                {currentMonth.balance.toLocaleString()} Mt
              </span>
            </div>
            ...
            <div className="relative h-32">
              <div className="absolute inset-0 flex items-end justify-between">
                {monthlyData.map((data, index) => {
                  const height =
                    (data.balance /
                      Math.max(...monthlyData.map(d => d.balance || 1))) *
                    100
                  return (
