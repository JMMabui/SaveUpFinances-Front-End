...
export function useCreateBank() {
  ...
  return useMutation({
    ...
    mutationFn: async (data: bankRequest) => apiService.post<bankResponde>(BASE, data),
    onSuccess: () => {
...
export function useUpdateBank(id: string) {
  ...
  return useMutation({
    ...
    mutationFn: async (data: Partial<bankRequest>) => apiService.put<bankResponde>(`${BASE}/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["banks"] });
