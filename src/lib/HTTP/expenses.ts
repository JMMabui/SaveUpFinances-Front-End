...
export function useGetExpensesByUser(userId: string) {
  return useQuery({
    ...
    queryFn: async () => apiService.get<expensesResponse[]>(`${BASE}/user/${userId}`),
    enabled: !!userId,
  });
}
...
export function useCreateExpense() {
  ...
  return useMutation({
    ...
    mutationFn: async (data: expensesRequest) => apiService.post<expensesResponse>(BASE, data),
    onSuccess: () => {
...
export function useUpdateExpense(id: string) {
  ...
  return useMutation({
    ...
    mutationFn: async (data: Partial<expensesRequest>) => apiService.put<expensesResponse>(`${BASE}/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["expenses"] });
